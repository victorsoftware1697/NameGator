    ...
    Mutation: {
        ...
        async generateDomains() {
            ...
            for (const prefix of items.filter(item => item.type === "prefix")) {
                for (const sufix of items.filter(item => item.type == "sufix")) {
                    ...
                    const checkout = `https://checkout.hostgator.com.br/?a=add&sld=${url}&tld=.com.br&domaincycle=2&mkt=builder&promocode=CRIADORDESITESTRIAL&`;
        ...
        async generateDomain(_, args) {
            ...
            for (const extension of extensions) {
                const url = name.toLowerCase();
                const checkout = `https://checkout.hostgator.com.br/?a=add&sld=${url}&tld=${extension}&domaincycle=2&mkt=builder&promocode=CRIADORDESITESTRIAL&`;
